...
function updateScale() {
    ...
    scale = Math.min(Math.max(5, scale), 50); 
    document.getElementById('scale').value = scale.toFixed(1);
    document.getElementById('scaleValue').textContent = `Масштаб: ${scale.toFixed(1)}`;
    redraw();
}
...
function plotPixel(x, y, color = '#ff0000', connect = true) {
    ...
    // Преобразуем координаты для рисования клеточек
    ...
    ctx.beginPath();
    ctx.fillStyle = color;
    ctx.rect(startX, endY, scale, scale);
    ctx.fill();

    // Если требуется соединение, рисуем линии между пикселями
    if (connect && plotPixel.lastX !== undefined && plotPixel.lastY !== undefined) {
        ...
        ctx.lineTo(startX + scale / 2, startY - scale / 2);
        ctx.stroke();
    }

    plotPixel.lastX = startX + scale / 2;
    plotPixel.lastY = startY - scale / 2;
}
...
function stepByStep(x1, y1, x2, y2) {
    ...
    if (Math.abs(dx) >= Math.abs(dy)) {  
        if (x1 > x2) {  
            [x1, y1, x2, y2] = [x2, y2, x1, y1]; //слева направо
        }
        const k = dx !== 0 ? dy / dx : 0;
        ...
        if (y1 > y2) { 
            [x1, y1, x2, y2] = [x2, y2, x1, y1]; // снизу вверх
        }
        const k = dy !== 0 ? dx / dy : 0; 
...
function DDA(x1, y1, x2, y2) {
    ...
    const xIncrement = dx / steps; //считаем инкременты по обоим осям
